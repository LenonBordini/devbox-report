...
module.exports = (config) => {
    ...
    return (req, res, next) => {
        res.pdf = async (origin, options) => {
            ...
            options.footer = options.footer || config.default.footer;
            options.css = css.concat(Array.isArray(options.css) ? options.css : []).map(x => `<link href="${x}" rel="stylesheet">`).join('');
            options.js = js.concat(Array.isArray(options.js) ? options.js : []).map(x => `<script src="${x}" type="javascript"></script>`).join('');
